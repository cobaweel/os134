 ...
 * Largely taken from work from CS134a, but with many changes.
 * Much of the inappropriate code is simply commented out, and
 * printf's have been replaced with p-rintf's, to act as "comments".
 *
 * It should, in theory, use the protected arrays, but I'm too
 * lazy to go through and change all of the instances.
 * Also note the horrific strcmp and strcpy used. Something better
 * should probably be done instead.a
 */
...
/* Replacement for perror, to avoid replacing it with "print" each time */
void perror(const char *err)
{
    print(err);
}

/* A few other replacement functions, as the drivers need to be written. */
int lseek(int fildes, int offset, int whence)
{
    return -1;
}

int open (const char *name, int type, int flags)
{
    return -1;
}
...
void msdos_print_dir_entry_name_override(MSDOSDirEntry *dirp, char *name_override)
{
    ...
    if (dirp->attr_archive) 
        ...
        p-rintf("-");
    p-rintf("  %s %02d, %02d; %02d:%02d:%02d  ", ts->monthStr, ts->day, ts->year, 
                                  ts->hour, ts->minute, ts->second);
    p-rintf("%11d  %-13s  -> %03x (%d)\n", dirp->filesize, out_name, dirp->first_cluster, dirp->first_cluster);
}
********/
...
int msdos_print_fat(MSDOS *dosp)
{
    ...
    for(i=0;i<dosp->fat_entries;i++)
        p-rintf("FAT Entry 0x%03lx:  0x%03x   (%ld:  %d)\n", i, dosp->fat_table[i], i, dosp->fat_table[i]);
    return 0;
}
********/
...
int msdos_chdir(MSDOS *dosp, const char *name)
{
    ...
    else if (strcmp("..", name) == 0)
    {
        ...
        {
            print("No parent of root directory.\n");
            return -1;
        }
...
int msdos_touch(MSDOS *dosp, const char *name)
{
    ...
    if (filep)
    {
        ...
        {
            print("No access permissions to file: %s", name);
            free(filep);
            return -1;
        }
        ...
        {
            print("Update error. Filesystem possibly corrupted.\n");
    ...
    if (add_dir_entry(dosp, filep))
    {
        print("Creation error. Filesystem possibly corrupted.\n");
        msdos_write_fat_table(dosp);
        free(buffer);
        free(filep);
        return -1;
...
int msdos_rm(MSDOS *dosp, const char *name)
{
    ...
    if (filep->attr_read_only == 1)
    {
        print("No access permissions for file: %s", name);
        free(filep);
        return -1;
    }
    ...
    {
        print("Deletion error. Filesystem possibly corrupted.\n");
        free(filep);
        msdos_write_fat_table(dosp);
        return -1;
    }
...
int msdos_rmdir(MSDOS *dosp, const char *name)
{
    ...
    if (filep->attr_read_only == 1)
    {
        print("No access permissions to directory: %s", name);
        free(filep);
        return -1;
    }
...
static void shell(MSDOS *dosp)
{
    ...
    while(1) 
    {
        ...
        p-rintf("%s:%s> ", dosp->root_dir->tname, dosp->current_dir->path);
        fflush(stdout);
        command[0] = 0;
        arg[0] = 0;
...
//            continue;
        ...
        if (strcmp(command, "boot") == 0)
            ...
            fp-rintf(stderr, "unknown command: %s\n", command);
    }
}
********/
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) 
    {
        fp-rintf(stderr, "usage: %s <devname>\n", argv[0]);
        return 1;
    }
    ...
    if (dosp == 0) 
    {
        fp-rintf(stderr, "%s: can't open %s\n", argv[0], argv[1]);
        return 1;
    }
